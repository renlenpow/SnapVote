...
//Use Url command to increment votes
switch($_GET['f']) {
case 'vote':
    add_new_vote($_GET['poll_id'],$_GET['votenum']);
    if($add_new_vote){
    header('Location: results.php?pollid='.$_GET['poll_id']);
    }
    else{
    print "Your phone has already voted for this Poll";
}
...
function add_new_vote($poll_id, $vote_value_id) {
	...
	$ipaddress = /*$_SERVER['REMOTE_ADDR'];*/ $_GET['sc'];
...
//create an IMEI record
function vote_IMEI_file_path($id) {
	return dirname(__FILE__) . '/data/IMEI_' . $id . '.dat';
}
